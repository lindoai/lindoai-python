...
# ----------------------------------------------------------------------------
# Used by client.workflows.create_website / edit_website / create_blog and the
# matching get_website_status / get_edit_status / get_blog_status pollers.
# All these create/edit endpoints return the same shape; the status endpoints
...
class CreateWorkflowResult:
    ...
    ``get_edit_status`` / ``get_blog_status`` to poll.
    """
...
class CreateWorkflowResponse:
    """
    Response returned by ``create_website``, ``edit_website``, and ``create_blog``.
...
class WebsiteWorkflowResult:
    ...
    """Business name of the website, if set."""

    domain: Optional[str] = None
    """
    Canonical URL where the website is reachable. Returns the verified custom
    domain if connected, otherwise the Lindo preview URL.
    """
...
@dataclass
class EditWebsitePageEntry:
    """A single page changed + published by a website-edit workflow."""

    path: str
    """URL path of the changed page, e.g. ``"/about"``."""
    page_id: Optional[str] = None
    """ID of the changed page, if known."""
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "EditWebsitePageEntry":
        return cls(
            path=data.get("path", ""),
            page_id=data.get("page_id"),
...
@dataclass
class EditWebsiteWorkflowResult:
    """Result payload for a website-edit workflow: which pages changed."""

    website_id: str
    pages_changed: int
    updated_pages: List[EditWebsitePageEntry] = field(default_factory=list)

    def to_dict(self) -> Dict[str, Any]:
        return {
            "website_id": self.website_id,
            "pages_changed": self.pages_changed,
            "updated_pages": [p.to_dict() for p in self.updated_pages],
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "EditWebsiteWorkflowResult":
        return cls(
            website_id=data.get("website_id", ""),
            pages_changed=data.get("pages_changed", 0),
            updated_pages=[
                EditWebsitePageEntry.from_dict(p)
                for p in data.get("updated_pages", [])
            ],
        )


@dataclass
class EditWebsiteStatusResult:
    """Inner result payload for website-edit workflow status responses."""
    ...
    message: str
    poll_after_ms: Optional[int] = None
    result: Optional[EditWebsiteWorkflowResult] = None
    error: Optional[str] = None
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "EditWebsiteStatusResult":
        raw_result = data.get("result")
        return cls(
            done=data.get("done", False),
            status=data.get("status", "running"),
            ...
            message=data.get("message", ""),
            poll_after_ms=data.get("poll_after_ms"),
            result=EditWebsiteWorkflowResult.from_dict(raw_result) if raw_result else None,
            error=data.get("error"),
        )
...
class EditWebsiteStatusResponse:
    """Response shape for GET /v1/ai/workspace/website/edit/status/{workflow_id}."""

    success: bool
    result: Optional[EditWebsiteStatusResult] = None
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "EditWebsiteStatusResponse":
        raw_result = data.get("result")
        return cls(
            success=data.get("success", False),
            result=EditWebsiteStatusResult.from_dict(raw_result) if raw_result else None,
        )
...
# ----------------------------------------------------------------------------
# Used by client.workflows.batch_create_websites / batch_create_blogs and the
# matching batch_check_*_status pollers.
