...
class WorkspaceResource:
    ...
    def get_credits(self) -> WorkspaceCreditsResponse:
        """
        Get the credit balance for the current workspace.

        Returns:
            The workspace credits response envelope with ``success`` and ``result``.

        Example:
            >>> response = client.workspace.get_credits()
            >>> if response.result:
            ...     print("Available:", response.result.balance.total_available)
        """
        data = self._http.get("/v1/ai/credits")
        return WorkspaceCreditsResponse.from_dict(data)

    def get_client_credits(self, client_id: str) -> ClientCreditsResponse:
        """
        ...
        Returns:
            The client credits response envelope with ``success`` and ``result``.
        """
        data = self._http.get("/v1/ai/credits/client", params={"client_id": client_id})
        return ClientCreditsResponse.from_dict(data)

    def allocate_client_credits(
        self,
        client_id: str,
        credit_type: str,
        amount: int,
        ...
        """
        Allocate credits from the workspace to a client.

        Args:
            client_id: The target client ID.
            credit_type: One of ``"monthly"``, ``"purchased"``, or ``"daily"``.
            amount: Number of credits to allocate (must be positive).
            source: Optional allocation source (e.g. ``"bonus"``, ``"purchase"``).
            notes: Optional notes for the allocation.

        Returns:
            The allocation response envelope with ``success`` and ``result``.
        """
        body: Dict[str, Any] = {
            "client_id": client_id,
        ...
        if notes is not None:
            body["notes"] = notes
        data = self._http.post("/v1/ai/credits/client/allocate", body)
        return AllocateClientCreditsResponse.from_dict(data)
...
class AsyncWorkspaceResource:
    ...
    async def get_credits(self) -> WorkspaceCreditsResponse:
        """
        Get the credit balance for the current workspace.

        Returns:
            The workspace credits response envelope.
        """
        data = await self._http.get("/v1/ai/credits")
        return WorkspaceCreditsResponse.from_dict(data)

    async def get_client_credits(self, client_id: str) -> ClientCreditsResponse:
        """
        Get the credit balance for a specific client.
        ...
        Returns:
            The client credits response envelope.
        """
        data = await self._http.get("/v1/ai/credits/client", params={"client_id": client_id})
        return ClientCreditsResponse.from_dict(data)

    async def allocate_client_credits(
        self,
        client_id: str,
        credit_type: str,
        amount: int,
        ...
        Args:
            ...
            credit_type: One of ``"monthly"``, ``"purchased"``, or ``"daily"``.
            amount: Number of credits to allocate (must be positive).
            source: Optional allocation source.
            notes: Optional notes for the allocation.

        Returns:
            The allocation response envelope.
        """
        body: Dict[str, Any] = {
            "client_id": client_id,
            "credit_type": credit_type,
        ...
        if notes is not None:
            body["notes"] = notes
        data = await self._http.post("/v1/ai/credits/client/allocate", body)
        return AllocateClientCreditsResponse.from_dict(data)
