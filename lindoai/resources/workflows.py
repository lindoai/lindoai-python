...
class WorkflowsResource:
    ...
    def create_website(
        ...
        Returns a ``workflow_id`` immediately — poll :meth:`get_website_status`
        with that id to track progress and get the final result.
        ...
        Returns:
            :class:`CreateWorkflowResponse`. Use ``response.workflow_id`` to poll.
        ...
        Example:
            ...
            >>> import time
            >>> while True:
            ...     status = client.workflows.get_website_status(resp.workflow_id)
            ...     if status.done:
    ...
    def edit_website(
        ...
        """
        Edit an existing website using AI.

        Starts an asynchronous workflow. The agent edits the site (across its
        pages) per the prompt. Poll :meth:`get_edit_status` with the returned
        ``workflow_id`` to track progress.

        Args:
            website_id: The website to edit.
            prompt: AI prompt describing the change to make (min 10 chars).
            publish: When True, publish the edited page(s) to the live website.
                When False or omitted, the changes are saved as drafts to review
                and publish from the editor ("Publish all").
            schedule_at: Optional ISO 8601 timestamp to schedule the edit for later.
        ...
        data = self._http.post(
            f"/v1/ai/workspace/website/{website_id}/edit", body
        )
        return CreateWorkflowResponse.from_dict(data)
    ...
    def create_blog(
        ...
        returned ``workflow_id`` to track progress.
    ...
    def get_website_status(
        ...
        Args:
            workflow_id: The ``workflow_id`` returned by :meth:`create_website`.

        Returns:
            :class:`WebsiteWorkflowStatusResponse`.
        """
        data = self._http.get(f"/v1/ai/workspace/website/status/{workflow_id}")
        return WebsiteWorkflowStatusResponse.from_dict(data)

    def get_edit_status(self, workflow_id: str) -> EditWebsiteStatusResponse:
        """
        Poll the status of a website-edit workflow started via :meth:`edit_website`.

        Args:
            workflow_id: The ``workflow_id`` returned by :meth:`edit_website`.

        Returns:
            :class:`EditWebsiteStatusResponse`.
        """
        data = self._http.get(f"/v1/ai/workspace/website/edit/status/{workflow_id}")
        return EditWebsiteStatusResponse.from_dict(data)

    def get_blog_status(self, workflow_id: str) -> BlogWorkflowStatusResponse:
        """
        Poll the status of a blog-creation workflow started via :meth:`create_blog`.

        Args:
            workflow_id: The ``workflow_id`` returned by :meth:`create_blog`.

        Returns:
            :class:`BlogWorkflowStatusResponse`.
        """
        data = self._http.get(f"/v1/ai/workspace/blog/status/{workflow_id}")
    ...
    def batch_create_websites(
        ...
        Returns:
            :class:`BatchCreateResponse` with per-item ``workflow_id`` / ``error``.
        ...
        Example:
            ...
            ... ])
            >>> ok_ids = [i.workflow_id for i in batch.items if i.success and i.workflow_id]
...
class AsyncWorkflowsResource:
    ...
    async def create_website(
        ...
        Starts an asynchronous workflow that generates a website from the prompt.
        Returns a ``workflow_id`` immediately — poll :meth:`get_website_status` with
    ...
    async def edit_website(
        ...
        """
        Edit an existing website using AI (async).

        Poll :meth:`get_edit_status` with the returned ``workflow_id``.
        """
        body: Dict[str, Any] = {"prompt": prompt}
        if schedule_at is not None:
            body["schedule_at"] = schedule_at
    ...
    async def create_blog(
        ...
        Poll :meth:`get_blog_status` with the returned ``workflow_id``.
        """
        body: Dict[str, Any] = {"prompt": prompt}
        if schedule_at is not None:
            body["schedule_at"] = schedule_at
    ...
    async def get_website_status(
        ...
        data = await self._http.get(
            f"/v1/ai/workspace/website/status/{workflow_id}"
        )
        return WebsiteWorkflowStatusResponse.from_dict(data)
    ...
    async def get_edit_status(
        ...
        """Poll the status of a website-edit workflow (async)."""
        data = await self._http.get(
            f"/v1/ai/workspace/website/edit/status/{workflow_id}"
        )
        return EditWebsiteStatusResponse.from_dict(data)
    ...
    async def get_blog_status(
        ...
        data = await self._http.get(
            f"/v1/ai/workspace/blog/status/{workflow_id}"
        )
        return BlogWorkflowStatusResponse.from_dict(data)
