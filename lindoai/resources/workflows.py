...
class WorkflowsResource:
    ...
    def get_page_status(self, workflow_id: str) -> PageWorkflowStatusResponse:
        """
        Poll the status of a page-creation workflow started via :meth:`create_page`.

        Args:
            workflow_id: The ``workflow_id`` returned by :meth:`create_page`.

        Returns:
            :class:`PageWorkflowStatusResponse`.
        """
        data = self._http.get(f"/v1/ai/workspace/page/status/{workflow_id}")
...
class AsyncWorkflowsResource:
    ...
    async def create_page(
        ...
        Poll :meth:`get_page_status` with the returned ``workflow_id``.
        """
        body: Dict[str, Any] = {"prompt": prompt}
        if schedule_at is not None:
            body["schedule_at"] = schedule_at
    ...
    async def get_page_status(
        ...
        data = await self._http.get(
            f"/v1/ai/workspace/page/status/{workflow_id}"
        )
        return PageWorkflowStatusResponse.from_dict(data)
