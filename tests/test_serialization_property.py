...
class TestWorkspaceCreditsRoundTrip:
    """
    Property tests for WorkspaceCredits serialization round-trip.
    ...
    **Validates: Requirements 6.7, 6.8**
    ...
    @given(
        ...
        balance=st.floats(min_value=0, max_value=1000000, allow_nan=False),
        allocated=st.floats(min_value=0, max_value=1000000, allow_nan=False),
        used=st.floats(min_value=0, max_value=1000000, allow_nan=False),
        reset_date=st.one_of(st.none(), iso_date),
    )
    def test_round_trip(
        self,
        workspace_id: str,
        ...
        **Property 17: Python SDK Serialization Round-Trip**
        ...
        For any valid WorkspaceCredits, to_dict() followed by from_dict()
        produces an equivalent instance.
        """
        original = WorkspaceCredits(
        ...
        serialized = original.to_dict()
        deserialized = WorkspaceCredits.from_dict(serialized)
        
        assert deserialized.workspace_id == original.workspace_id
        assert deserialized.balance == original.balance
        assert deserialized.allocated == original.allocated
        assert deserialized.used == original.used
